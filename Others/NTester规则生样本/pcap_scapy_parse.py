...
def get_http_urls(pcap_file_path):
    """
    ...
    for each_packet in parse_data:
        if is_http_packet(each_packet):
            url = get_url_from_raw_data(get_raw_info(each_packet))
            urls.append(url) if url else None
    print(urls)


if __name__ == "__main__":
    file_path = "13120018.pcap"
    get_http_urls(file_path)
